...
void calculateTransformationParameters(const int n, const double* srcPoints, const double* dstPoints, double& dx, double& dy, double& scale, double& rotation);//// 计算四参数转换 
void transformPoint(const double x, const double y, double& rex, double& rey, double dx, double dy, double scale, double rotation);// 使用四参数进行坐标转换 
